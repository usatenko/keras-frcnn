...
def convert_xml_to_json(input_path, output_path, difficult):
	...
	outfile = join(output_path, 'all.csv')
	...
	for file in onlyfiles:
		...
		if validate_metadata(trimmed, file):
			for k in trimmed['object']:
				r = trimmed['filename'] + ',' + str(k['bndbox']['xmin']) + ',' + str(k['bndbox']['ymin']) + ',' + str(k['bndbox']['xmax']) + ',' + str(k['bndbox']['ymax']) + ',' + k['name']
				f.write(r + "\n")
		else:
